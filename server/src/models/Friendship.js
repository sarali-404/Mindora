...
friendshipSchema.statics.getFriends = async function(userId) {
  ...
  .populate('requester', 'username profile.firstName profile.lastName profile.avatar isOnline lastSeen')
  .populate('recipient', 'username profile.firstName profile.lastName profile.avatar isOnline lastSeen');
...
friendshipSchema.statics.getPendingRequests = async function(userId) {
  ...
  .populate('requester', 'username profile.firstName profile.lastName profile.avatar')
  .sort({ createdAt: -1 });
};
