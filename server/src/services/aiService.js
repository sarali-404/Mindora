 ...
 * @param {string} knowledgeContext - Optional ML knowledge context for adaptive content
 */
async function generateNotes(text, topicName, subject, knowledgeContext = '') {
  const truncatedText = truncateText(text, TEXT_LIMITS.NOTES);

  const adaptiveInstruction = knowledgeContext
    ? `\n\nADAPTIVE LEARNING CONTEXT:\n${knowledgeContext}\nAdjust the depth, complexity, and examples in the notes based on the student's knowledge level above.\n`
    : '';

  const prompt = `You are an expert educational content creator. Generate comprehensive study notes for the topic "${topicName}" in the subject "${subject}".${adaptiveInstruction}
 ...
 * @param {string} knowledgeContext - Optional ML knowledge context for adaptive content
 */
async function generateSummary(text, topicName, subject, knowledgeContext = '') {
  const truncatedText = truncateText(text, TEXT_LIMITS.SUMMARY);

  const adaptiveInstruction = knowledgeContext
    ? `\n\nADAPTIVE LEARNING CONTEXT:\n${knowledgeContext}\nFocus the summary on areas where the student needs reinforcement.\n`
    : '';

  const prompt = `You are an expert educational summarizer. Create a concise but comprehensive summary of "${topicName}" in "${subject}".${adaptiveInstruction}
 ...
 * @param {string} knowledgeContext - Optional ML knowledge context for adaptive difficulty
 */
async function generateQuiz(text, topicName, subject, difficulty = 'medium', questionCount = 10, knowledgeContext = '') {
  ...
  const adaptiveInstruction = knowledgeContext
    ? `\n\nADAPTIVE LEARNING CONTEXT:\n${knowledgeContext}\nAdjust question difficulty and focus areas based on the student's knowledge profile above. Emphasize their weak areas while still testing strong areas.\n`
    : '';
...
Number of questions: ${questionCount}${adaptiveInstruction}
 ...
 * @param {string} knowledgeContext - Optional ML knowledge context for adaptive content
 */
async function generateEssayQuestions(text, topicName, subject, difficulty = 'medium', questionCount = 5, knowledgeContext = '') {
  const truncatedText = truncateText(text, TEXT_LIMITS.ESSAY);

  const adaptiveInstruction = knowledgeContext
    ? `\n\nADAPTIVE LEARNING CONTEXT:\n${knowledgeContext}\nTailor essay questions to probe the student's weak areas while building on their strengths.\n`
    : '';
